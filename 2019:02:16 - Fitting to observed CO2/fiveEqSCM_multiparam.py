...
def multiscen_oxfair(emissions,
    ...
    if other_rf == np.ones(100)*-99:
        other_rf = np.zeros(emissions.shape[-1])
...
#######
    ...
    RF[...,0] = step_forc(C=C[...,0],pre_ind_C=PI_C,F_ext=other_rf[0],f=f)
    ...
    for t in np.arange(1,emissions.shape[2]):
        ...
        RF[...,t] = step_forc(C=C[...,t],pre_ind_C=PI_C,F_ext=other_rf[t],f=f)
